...
只维护直接位于 `.agents/skills/` 下的 `ctx-*` skill 及其内部资源。不修改 `project-context`、非 `ctx-*` skill、项目代码、配置、Git 历史或 `docs/`。
...
按知识边界拆分候选内容。每项候选内容应包含用户意图、证据来源，以及可能相关的既有 `ctx-*`。
...
以 `project-context：` 开头汇总结果。列出实际新增、更新或删除的 `ctx-*`、内部 resource 和项目基础上下文，并为每项给出证据。另列出待确认、跳过和保持不变的内容；结构调整尚未确认时，说明已输出的计划及未执行原因。
