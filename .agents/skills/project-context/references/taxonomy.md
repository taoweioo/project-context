...
- 分类名称的中文表达可以在不改变职责边界的前提下优化；固定英文分类标识一旦已用于真实项目中的 `ctx-*` 路径，不得修改、替换或复用。
- 不得通过普通内容维护、重命名或移动，将一个已存在的 `ctx-*` 改用另一套分类标识。
- 修改、合并、删除或替换固定英文分类标识属于**分类体系迁移**，不属于普通结构调整。
- 分类体系迁移必须先审计该项目全部 `ctx-*` 上下文单元，列出受影响的路径、frontmatter `name`、关联链接和规则归属，再输出完整迁移计划并等待用户明确确认。
- 本机制当前不执行分类体系迁移；需要这类迁移时，应先单独扩展流程规则。
...
只有同时满足以下条件，才可以新增顶层分类：

1. 该知识无法稳定归入现有分类。
2. 不止一个真实前端项目会反复产生这类独立上下文单元。
3. 它有明确、独立的触发词和职责边界。
4. 继续使用现有分类会持续造成重复、误触发或职责膨胀。
5. 不能只因为项目中存在某个目录、框架、工具或单次需求就新增分类。
