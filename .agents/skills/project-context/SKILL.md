...
description: 当用户明确要求初始化、维护或审计前端项目的持久化上下文，或询问、优化 project-context 自身设计时使用。负责维护 `.agents/skills/ctx-*` skill 及其内部资源，可根据代码、git diff、项目说明和历史问题处理长期项目知识。不用于普通功能开发、代码修改后的自动更新、后端规则提炼或维护非 `ctx-*` skill。
---
...
- **设计说明**：介绍或优化 `project-context` 自身的职责和调用关系，不作为 `ctx-*` 业务流程执行。

## 按场景读取

- 用户询问或优化本机制设计时，读取 [设计说明](references/design.md)。
